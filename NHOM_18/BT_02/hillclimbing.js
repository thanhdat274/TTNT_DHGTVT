...
function docDoThiTuFile(filename) {
  ...
  for (let i = 0; i < duLieu.length; i++) {
    ...
    if (phanTu[0] === 'START') {
      ...
      // Đọc các cạnh với trọng số (tu, den, trongSo1)
      const [tu,trongSo, den, trongSo1] = phanTu;
      if (!doThi[tu]) {
        doThi[tu] = [];
      }
      doThi[tu].push({ den, trongSo1: parseInt(trongSo1, 10) }); // Lưu đỉnh và trọng số
    }
...
function hillClimbing(doThi, batDau, ketThuc) {
  ...
  while (L.length > 0) {
    ...
    for (const { den, trongSo1 } of neighbors) {
      if (!visited.has(den)) {
        L1.push({ den, trongSo1 });
      }
    }
    
    // Sắp xếp L1 theo thứ tự tăng dần của trọng số
    L1.sort((a, b) => a.trongSo1 - b.trongSo1);
    ...
    result.push({
      TT: TT++,
      TTPT: u,
      TTKE: neighbors.map(n => `${n.den} (${n.trongSo1})`).join(', '),
